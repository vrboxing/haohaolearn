...
.    "BEAM188")     ("BEAM54"    .     "BEAM188")    ("CONTACT12"
.    "Contac178")     ("COMBIN7"    .     "MPC184")    ("FLUID79"
.     "Fluid29")    ("FLUID80"     .    "Fluid29")     ("FLUID81"
. "Fluid29") ("FLUID141" .  "CFX") ("FLUID142" . "CFX") ("INFIN9"
.    "INFIN110")    ("INFIN47"     .    "INFIN111")    ("PLANE13"
...
ASIFILE, Opt, Fname, Ext" "ASKIN - Generates an area by \"skinning\" a surface through guiding lines.
...
CAMPBELL, Key, NSOLVE" "CBDOF - Activates cut-boundary interpolation (for submodeling).
...
CNCHECK, Option, RID1, RID2, RINC, PairType, TRlevel" "CNKMOD - Modifies contact element key options.
...
CVAR, IR, IA, IB, ITYPE, DATUM, Name" "CYCFILES - Specifies the data files where results are to be found for a cyclic harmonic mode-superposition analysis.
...
CYCFREQ, Option, Value1, Value2" "CYCLIC - Specifies a cyclic symmetry analysis.
...
DFLX, NODE, BX, BY, BZ, BX2, BY2, BZ2" "DIG - Digitizes nodes to a surface.
...
DUMP, NSTRT, NSTOP" "DVAL - Defines values at enforced base.
...
FINISH - Exits normally from a processor." "FIPLOT - Plots a FiberSIM model.
FIPLOT, Option, Filename, Ext, Dir, REQLAYER, FSPRINT" "FITEM - Identifies items chosen by a picking operation (GUI).
...
HARFRQ, FREQB, FREQE, --, LogOpt" "HBMAT - Writes an assembled global matrix in Harwell-Boeing format.
...
ICDELE - Deletes initial conditions at nodes." "ICE, ELEM, Lab, VALUE
ICEDELE, ELEM, Lab" "ICLIST - Lists the initial conditions.
...
MSHPATTERN, KEY" "MSTOLE - Adds two extra nodes from FLUID116 elements to SURF151 or SURF152 elements for convection analyses.
...
NLIST, NODE1, NODE2, NINC, Lcoord, SORT1, SORT2, SORT3, KINTERNAL" "NLOG - Forms the natural log of a variable.
...
OUTRES, Item, Freq, Cname, -- , NSVAR" "OVCHECK - Checks for overconstraint among constraint equations and Lagrange multipliers.
...
PHYSICS, Option, Title, Fname, Ext, --" "PILECALC - Invokes soil-pile calculations.
PILECALC, IEL, KEYKM, KeyOut, KeyPl, LCPILE, StruPile" "PILEDISPSET - Sets up pile cap displacement data for soil-pile analysis.
PILEDISPSET, NODE, NDCAP, FnameCap, ExtCap" "PILEGEN - Generates data for elements used in soil-pile analysis.
PILEGEN, IEL, MAT, SECID, SOILNUM, SWOPT, CHECK, YIELD, LOADTYPE, SUBDIVISION, DEPTH, CONVERGE, ITER, PDELTA, STIFF, LCPILE, StruPile" "PILELOAD - Applies pile cap loads to the specified node.
PILELOAD, NODE, StruPile, CAPNODE, LCPILE, KeyPl" "PILEMASS - Gets pile cap mass and applies it to the specified element.
PILEMASS, IEL, StruPile, CAPNODE, LCPILE" "PILERUN - Runs a soil-pile analysis.
PILERUN, CAPNODE, FnamePile, ExtPile, ExtOut, KeyOut" "PILESEL - Selects all pile elements.
PILESEL, TYPE" "PILESTIF - Gets pile cap stiffness and applies it to the specified element.
PILESTIF, IEL, StruPile, NDCAP, LCPILE" "PIVCHECK - Controls the behavior of an analysis when a negative or zero equation solver pivot is encountered.
PIVCHECK, KEY, PRNTCNTRL" "PLCAMP - Plots Campbell diagram data for applications involving rotating structure dynamics.
PLCAMP, Option, SLOPE, UNIT, FREQB, Cname, STABVAL, --, KeyNegFreq" "PLCINT - Plots the fracture parameter (CINT) result data.
...
PLLS, LabI, LabJ, Fact, KUND" "PLMAP - Plots target and source pressures.
PLMAP, Item, --, NODEkey" "PLNEAR - Plots the pressure in the near zone exterior to the equivalent source surface.
...
PLSECT, Item, Comp, RHO, KBR" "PLST - Plots sound power parameters vs. frequency.
PLST, Fname, Ext, Lab" "PLTIME - Defines the time range for which data are to be displayed.
...
PLVECT, Item, Lab2, Lab3, LabP, Mode, Loc, Edge, KUND" "PMAP - Creates mapping of the path geometry by defining path interpolation division points.
PMAP, FORM, DISCON" "PMGTRAN - Summarizes electromagnetic results from a transient analysis.
PMGTRAN, Fname, FREQ, Fcnam1, Fcnam2, Pcnam1, Pcnam2, Ecnam1, Ccnam1" "PMLOPT - Defines perfectly matched layers (PMLs) for a high-frequency acoustic analysis.
...
QDVAL, TBLNO1, TBLNO2, SV1, SV2, SV3, SV4, SV5, SV6, SV7" "QRDOPT - Specifies additional QRDAMP modal analysis option.
QRDOPT, ReuseKey,-,-,-,CMCCoutKey" "QSOPT - Specifies quasi static radiation options.
...
SNOPTION, RangeFact, BlockSize, RobustLev, Compute, --, Solve_Info" "SOLCONTROL - Specifies whether to use optimized nonlinear solution defaults and some enhanced internal solution algorithms.
...
TRNOPT, Method, MAXMODE, --, MINMODE, MCout, TINTOPT" "TRPDEL - Deletes particle flow or charged particle trace points.
...
WTBCREATE, IEL, NODE, DAMP" "XVAR - Specifies the X variable to be displayed.
...
~PROEIN, Name, Extension, Path, ProecommFMT" "~SATIN - Transfers a .SAT file into the ANSYS program.
...
ARSPLIT" "GAPFINISH - Ansys undocumented command
...
OPRSW" "PLVAROPT - Ansys undocumented command
...
PGSAVE" "TOTAL - Ansys undocumented command
...
"\\(?:\\*\\(?:EVAL\\|MOONEY\\)\\|/\\(?:OPT\\|P\\(?:ICE\\|METH\\)\\|RUNST\\(?:AT\\)?\\|SSS\\|TRACK\\|V\\(?:ERIFY\\|T\\)\\|WB\\|XML\\)\\|A\\(?:LPFILL\\|R\\(?:COLLAPSE\\|DETACH\\|FILL\\|MERGE\\|SPLIT\\)\\)\\|C\\(?:MBLOCK\\|NTR\\|WZPLOT\\)\\|E\\(?:ALL\\|BLOCK\\)\\|FL\\(?:DATA1-40\\|ITEM\\|O\\(?:CHECK\\|TRAN\\)\\|READ\\)\\|GAP\\(?:FINISH\\|LIST\\|MERGE\\|\\(?:OP\\|PLO\\)T\\)\\|HF\\(?:A\\(?:DP\\|RRAY\\)\\|DEEM\\|E\\(?:IGOPT\\|REFINE\\)\\|MODPRT\\|P\\(?:A\\|CSWP\\|O\\(?:RT\\|WER\\)\\)\\|SCAT\\)\\|IC\\(?:E\\(?:DELE\\|LIST\\)?\\|VFRC\\)\\|L\\(?:N\\(?:COLLAPSE\\|DETACH\\|FILL\\|MERGE\\|SPLIT\\)\\|PRT\\)\\|MS\\(?:ADV\\|CAP\\|DATA\\|M\\(?:ASS\\|ETH\\|IR\\)\\|NOMF\\|PROP\\|QUAD\\|RELAX\\|S\\(?:OLU\\|PEC\\)\\|TERM\\|VARY\\)\\|N\\(?:ALL\\|BLOCK\\|ELE\\)\\|OP\\(?:A\\(?:DD\\|NL\\)\\|CLR\\|D\\(?:ATA\\|EL\\)\\|E\\(?:QN\\|XE\\)\\|F\\(?:\\(?:AC\\|RS\\)T\\)\\|GRAD\\|KEEP\\|L\\(?:FA\\|GR\\|IST\\|OOP\\|SW\\)\\|MAKE\\|PRNT\\|R\\(?:AND\\|ESU\\|FA\\|GR\\|SW\\)\\|S\\(?:AVE\\|EL\\|\\(?:UB\\|WEE\\)P\\)\\|TYPE\\|\\(?:USE\\|VA\\)R\\)\\|P\\(?:CONV\\|E\\(?:MOPTS\\|RI\\|XCLUDE\\)\\|G\\(?:R\\(?:APH\\|SET\\)\\|\\(?:S\\(?:AV\\|EL\\)\\|WRIT\\)E\\)\\|INCLUDE\\|L\\(?:CONV\\|FSS\\|S\\(?:CH\\|YZ\\)\\|T\\(?:D\\|LINE\\)\\|V\\(?:AROPT\\|FRC\\)\\|WAVE\\)\\|M\\(?:ETH\\|OPTS\\)\\|OUTRES\\|P\\(?:LOT\\|RANGE\\)\\|R\\(?:CONV\\|ECISION\\|SYZ\\|VAROPT\\)\\)\\|QFACT\\|R\\(?:ALL\\|FILSZ\\|ITER\\|MEMRY\\|S\\(?:PEED\\|TAT\\)\\|\\(?:TIMS\\|WFRN\\)T\\)\\|S\\(?:ARPLOT\\|HSD\\|L\\(?:[PS]PLOT\\)\\|P\\(?:A\\(?:DP\\|RM\\)\\|FSS\\|ICE\\|S\\(?:CAN\\|WP\\)\\)\\)\\|T\\(?:O\\(?:COMP\\|DEF\\|EXE\\|FREQ\\|GRAPH\\|L\\(?:IST\\|OOP\\)\\|P\\(?:\\(?:LO\\|RIN\\)T\\)\\|STAT\\|T\\(?:AL\\|YPE\\)\\|VAR\\)\\|Z\\(?:AMESH\\|DELE\\|EGEN\\)\\)\\|V\\(?:CVFILL\\|T\\(?:CLR\\|DISC\\|EVAL\\|FREQ\\|GEOM\\|IN\\|M\\(?:ETH\\|P\\)\\|OP\\|POST\\|R\\(?:EAL\\|FIL\\|SLT\\)\\|S\\(?:EC\\|FE\\|L\\|TAT\\)\\|TEMP\\|VMOD\\)\\)\\|X\\(?:MLO\\|VAROPT\\)\\|~\\(?:CFIN\\|EUI\\)\\)"
